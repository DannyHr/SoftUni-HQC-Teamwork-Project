...
namespace RestSharp.Deserializers
{
    ...
    public class XmlDeserializer : IDeserializer
    {
        ...
        public virtual T Deserialize<T>(IRestResponse response)
        {
            ...
            {
                x = (T)this.HandleListDerivative(x, root, objType.Name, objType);
            }
            else
            {
                x = (T)this.Map(x, root);
        ...
        protected virtual object Map(object x, XElement root)
        {
            ...
            foreach (var prop in props)
            {
                ...
                else if (type == typeof(DateTime))
                {
                    ...
                    {
                        value = DateTime.ParseExact(value.ToString(), this.DateFormat, this.Culture);
                    }
                    else
                    {
                        value = DateTime.Parse(value.ToString(), this.Culture);
                    }
                ...
                else if (type.IsSubclassOfRawGeneric(typeof(List<>)))
                {
                    ...
                    var list = this.HandleListDerivative(x, root, prop.Name, type);
                    prop.SetValue(x, list, null);
                }
                else
                {
